...
<script>
	export default {
		...
		async onShow(options) {
			...
			let cleanType = uni.getStorageSync('discussdiscussfuwuxiangmuCleanType')
			if(cleanType){
				uni.removeStorageSync('discussdiscussfuwuxiangmuCleanType')
				this.mescroll.num = 1
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('discussfuwuxiangmu', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','discussfuwuxiangmu');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','discussfuwuxiangmu');
			...
			async init(){
				...
				let res = await this.$api.info('discussfuwuxiangmu', this.id);
				this.detail = res.data;
			},
			...
			async upCallback(mescroll) {
                let res = await this.$api.list('discussdiscussfuwuxiangmu', {
					page: mescroll.num,
					limit: 10,
					refid: this.id
				});
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discussdiscussfuwuxiangmu/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
